...
class RegressionTree:
    ...
    def _build_tree(self, X_train, y_train, sample_idx, depth):
        ...
        node.left_child = self._build_tree(
            X_train, y_train, sample_idx[left_idx], depth + 1
        )
        node.right_child = self._build_tree(
            X_train, y_train, sample_idx[right_idx], depth + 1
        )
    ...
    def _find_best_split(self, X_train, y_train):
        ...
        splits_df = pd.DataFrame(
            [
                self._split_feature(X_train[:, col], y_train)
    ...
    def _split_feature(self, x_train, y_train):
        ...
        cumsum_y_sq = np.cumsum(
            y_train_sorted**2
        )  # cumulative sum of squared target values
